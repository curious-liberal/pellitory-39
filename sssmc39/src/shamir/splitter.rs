...
impl Splitter {
	...
	pub fn split_secret(
		...
		if shared_secret.len() < 16 || !shared_secret.len().is_multiple_of(2) {
	...
	fn check_digest(
		...
		let result = if digest != self.create_digest(&random_part, &shared_secret.share_value) {
			Err(ErrorKind::Digest(
