...
///
/// SECURITY: the packed bits hold `identifier || ext||exp || group/member
/// indices || share_value || checksum`. The `share_value` portion is
/// secret-equivalent above the threshold, so the backing `BitVec` is wiped
/// on drop via the custom [`Drop`] impl below. Ordinary `BitVec`/`Vec`/// `Drop` only frees the heap allocation without overwriting it, leaving the
/// secret bits recoverable in freed memory until the allocator reuses the
/// page.
...
impl Drop for BitPacker {
	fn drop(&mut self) {
		// Zeroise every in-range bit. `BitVec` is backed by machine words
		// (default `usize`); setting each bit to `false` writes a zero into
		// the in-range portion of every backing word, wiping the packed
		// share value. Capacity slack beyond `len()` is allocator-owned
		// uninitialized space, never written by the packing operations, so
		// it holds no secret material.
		for mut bit in self.bv.iter_mut() {
			*bit = false;
		}
	}
}
...
impl BitPacker {
	...
	pub fn normalize(&mut self, radix: usize) {
		while !self.bv.len().is_multiple_of(radix) {
			self.bv.pop();
		}
	}
